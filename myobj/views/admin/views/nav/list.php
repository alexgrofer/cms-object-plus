...
foreach($listobjsort as $objarr) {
	$obj = $objarr['obj'];
	$uihtml = '';
	if($arrayuirow['edit']) $uihtml .= ' <a href="'.$this->createUrl($arrayuirow['edit'].$obj->primaryKey).'"><i class="icon-edit"></i></a>';
	if($arrayuirow['links']) $uihtml .= ' | <a href="'.$this->createUrl($arrayuirow['links'].$obj->primaryKey).'">links</a>';

	$objclass = uClasses::getclass(array('navigation_sys','param_sys'));
	$uihtml .= ' | <a href="'.$this->createUrl('objects/class/'.$objclass['param_sys']->primaryKey.'/action/lenksobjedit/'.$obj->primaryKey.'/class/'.$objclass['navigation_sys']->primaryKey).'">params</a>';
	if($arrayuirow['edittempl']) $uihtml .= ' | <a class="btn btn-primary" href="'.$this->createUrl($arrayuirow['edittempl'].$obj->primaryKey, array('usercontroller'=>'usernav')).'">template</a>';

	$visibl = ($obj->show)?'+':'-';

	$uihtml .= ' | --- <a onclick="return confirm(\'remove id - '.$obj->primaryKey.'\')" href="'.$this->createUrl($arrayuirow['remove'].$obj->primaryKey).'"><i class="icon-remove"></i></a>';
